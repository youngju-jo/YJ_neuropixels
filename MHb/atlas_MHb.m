function atlas_MHb(cohort_flat, plot_optotagged, optotagged)
...
for ii = 1:5
    ...
    is_MHb = within_interval.*(cohort_flat.region==483);
    is_LHb = within_interval.*(cohort_flat.region==186);
    is_elsewhere = within_interval.*(cohort_flat.region~=483).*(cohort_flat.region~=186);
