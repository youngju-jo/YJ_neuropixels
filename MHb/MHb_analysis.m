...
elseif cohort_type == 3
    optotagged = logical((cohort_flat.salt_I > 0.2) .* (cohort_flat.spike_probability > 0.3) .* (cohort_flat.spike_latency < 0.020)); disp(sum(optotagged));
end
...
% figure, plot(mean(isnan(spike_BxtxN), [2,3]));  
num_last_trials = 60;
...
rate_BxN = squeeze(nanmean(rate_BxtxN(500-num_last_trials:end, 400:600,:), 2));  % also try 400:600 reward period
