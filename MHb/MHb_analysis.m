...
%% across-trial changes during optotagging
...
num_last_trials = 20;  % 70 for regular sessions, ~200 for stim sessions? // 40 for perturbation session 3, 20 for perturbation session 4
...
ylabel('Stim trials from session end');
...
% save('C:\Users\YoungJu Jo\Dropbox\Ephys\MHb_data\ramping_MHbDorsal.mat', 'rate_Bxt', 'rate_BxN', 'ramping_N', '-v7.3');


%% ramping summary

list_pop = {'MHbTac1', 'MHbDorsal', 'MHb', 'LHbTac1', 'LHb', 'PVTTac1', 'PVT', 'MS', 'TRS'};
...
% Run stat test on the linear fitting data first

% perhaps show curve (too noisy for overlap; do linear fit and calculate p value based on confidence interval) and a single pie chart for only MHb Tac1
% then show ramping summary
...
%% DONE: peak-sorted activity across regions (set cond==ones)
...
%% DONE: firing rate plot (export for seabon)
